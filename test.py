...
def monte_carlo(gamma, ro, pi_1, n_iterations, N_obs, sigma_z, r):
    ...
    for i in range(n_iterations):
        ...
        if math.isclose(d_Var, 0, rel_tol=10e-4):
            d_Var_psinv = 0
        else:
            d_Var_psinv = 1 / d_Var
        ...
        if Hausman < 0:
            print(b_hat_ols, b_hat_2sls, Var_b_ols, Var_b_2sls, conc_par)
...
def outcome(gamma, ro, pi_1, n_iterations, N_obs, sigma_z, r):
    Hausman_res1, parameters = monte_carlo(gamma, ro, pi_1, n_iterations, N_obs, sigma_z, r)
    ...
    plt.show()
    print(Hausman_res1[np.where(Hausman_res1[:, 0] < 0)])
# def outcome(gamma, ro, pi_1, n_iterations, N_obs, sigma_z, r):
